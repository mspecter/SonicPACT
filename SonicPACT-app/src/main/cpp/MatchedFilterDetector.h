...
public:
    ...
    uint64_t last_broadcast_seen_finished = 0;
    uint64_t last_recv_seen = 0;

    void setIsLeader(bool isLeader);

    // Thread-safe singleton for MFD
    static MatchedFilterDetector& getInstance(float sampleRate, bool isLeader){
        LOGE("MatchFilterDetector is Leader %d", isLeader);
        static MatchedFilterDetector instance;
...
private:
    ...
    void init(std::vector<float>& broadcast_wave, std::vector<float>& recv_wave){
        ...
        SAMPLES_PER_BROADCAST_FLT = sizeof(float)*SAMPLES_PER_BROADCAST;
        ...
        dumpBuffer(&recv_wave_to_match, "/sdcard/recv.txt");
        dumpBuffer(&broadcast_wave_to_match, "/sdcard/bcast.txt");

        should_continue_detecting = true;
        // kick off nanny thread:
        myThread = new std::thread(&MatchedFilterDetector::runNannyThread, this);
    }

    void dumpBuffer(cv::Mat* mat, std::string filename);
    ...
    struct match_result {
        ...
        float match_max; // max matched value
    ...
    cv::Mat wave_buffers[3] = {cv::Mat::zeros(1, BUFF_LEN, CV_32F),
...
int current_buffer = 0;
    ...
    bool isLeader;
    size_t SAMPLES_PER_BROADCAST_FLT;
    float noise;
};
